...
/**
 * Class de base pour les http exception
 * @package HttpException
 */
abstract class HttpException extends Exception
{
    /** @var string[] liste des codes d'état */
    protected static $messageStatus = [
        // CE NE SONT PAS DES EXCEPTION
        ...
        // 208 => "AlreadyReported", //   WebDAV : Le document a été envoyé précédemment dans cette collection.
        // 210 => "Content Different",//     WebDAV : La copie de la ressource côté client diffère de celle du serveur (contenu ou propriétés).
        // 226 => "I'm Used", //   RFC 32296 : Le serveur a accompli la requête pour la ressource, et la réponse est une représentation du résultat d'une ou plusieurs manipulations d'instances appliquées à l'instance actuelle.
    ...
    {
        $codes = array_keys(self::$messageStatus);
        if (in_array($this->code, $codes, false)) {
            return self::$messageStatus[$this->code];
        }
        return self::$messageStatus[500];
    }
}
